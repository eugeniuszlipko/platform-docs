...
таблиц нет — редактор скрыт (`contentRelationsEnabled(site)` по `site.schema`).

- Данные — `src/lib/contentRelations.ts` (`getRelations` / `setRelations`,
  replace-all, максимум 3 на блок). Это ОБЩИЙ слой всех блоков рекомендаций
  (посты, рецепты, товары), с реестром связей `MODELS`.
- UI — `src/features/shared/RelationsEditor.tsx`, тот же компонент, что у пинов
  «Read also» блога и рецепта: порядок ↑/↓, удаление, добавление через
  существующие пикеры (`ProductPickerDialog`, `RecipePickerDialog`,
  `PostPickerDialog`), кнопки Pin исчезают на лимите. Товар не может ссылаться
  сам на себя (в БД `check`) — свой id исключён из пикера; уже закреплённые id
  тоже исключены (unique-констрейнта на полиморфные колонки нет).
- Состояние — вне RHF (как категории товара): хук
  `src/features/shared/useRelationField.ts` с собственным dirty-флагом, который
  учитывает гард несохранённых изменений; запись идёт после сохранения самой
  строки товара (у нового товара — после create).
- ⚠️ **Пустой список пинов больше НЕ значит «блок скрыт»**: с 2026-08-26 сайт
  добирает блок авто-подбором и детерминированным фолбэком (см.
  [../sites/avocado-kiss.md](../sites/avocado-kiss.md) §13), поэтому подпись в UI —
  «Optional — auto-filled if empty». Неопубликованный рецепт остаётся пином, но
  на сайте отфильтровывается и заменяется другим материалом — блок всё равно
  показывает 3 карточки.
