...
- UI: `src/features/shared/RelationsEditor.tsx` — ОБЩИЙ компонент пинов для
  блога, рецептов и товаров (с 2026-08-26; заменил отдельный `RelatedEditor`).
  Список пинов с пикерами «Pin recipe» / «Pin post» + ↑/↓/✕. Значение приходит
  из RHF-поля `related` формы статьи через `Controller`, сохраняет его
  `syncRelated` вместе с постом — поведение блога не изменилось.
- С 2026-08-26 авто-подбор считает общий резолвер сайта (`lib/relations.ts`):
  пины → скоринг по тегам → детерминированный случайный фолбэк, поэтому блок
  больше не бывает куцым. Подробности — [../sites/avocado-kiss.md](../sites/avocado-kiss.md) §13.
