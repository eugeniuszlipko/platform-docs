...
> Last updated: 2026-08-27 | Source project: web.admin (docs/ux-ui.md, актуально на 2026-07-16) — пути `src/…` относятся к репозиторию `web.admin/`
...
- **Ручные пины блоков рекомендаций** — ОДИН компонент на все разделы:
  `src/features/shared/RelationsEditor.tsx` (+ хук `useRelationField`, слой данных
  `src/lib/contentRelations.ts`). Параметризуется `sourceKind` + `slot`, остальное
  (какие типы можно закрепить, лимит) берёт из общего реестра связей. Подключён к
  постам, товарам и рецептам Avocado Kiss; у cozycorner скрыт
  (`contentRelationsEnabled` по `site.schema`). Новый блок рекомендаций = строка в
  реестре, а не новый редактор. ⚠️ Пустой список пинов означает **авто-подбор на
  сайте**, а не скрытый блок — так и подписано в UI («Optional — auto-filled if
  empty»); не возвращать формулировки вида «блок скрыт, пока пусто».
