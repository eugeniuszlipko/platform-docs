...
| Avocado Kiss | `avocado.kiss/` | `avocado_kiss` / `avocado-kiss-photos` | https://www.avocadokiss.com | [avocado-kiss.md](avocado-kiss.md) |
...
- **Прод-адрес сайта зашит НЕ в одном месте.** Как минимум: `NEXT_PUBLIC_SITE_URL`
  в окружении Vercel (инлайнится в билд → нужен редеплой; питает `sitemap.ts` и
  `robots.ts`), `frontendUrl` записи сайта в `web.admin/src/config/sites.ts`
  (ссылки из админки на фронт, напр. превью черновика) и — если сайт использует
  Cloudflare Turnstile — список разрешённых хостнеймов виджета. Смена домена без
  прохода по всем пунктам ломает вещи тихо. Готовый чек-лист для avocado.kiss —
  [avocado-kiss.md](avocado-kiss.md) §7.1; для остальных сайтов набор тот же.
