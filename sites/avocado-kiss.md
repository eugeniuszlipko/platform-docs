...
- **Прод сейчас — `https://www.avocadokiss.com`** (кастомный домен подключён
  ...
  ⚠️ **Чек-лист §7.1 почти закрыт**: домен подключён в Vercel (п.1), Cloudflare
  Turnstile hostname добавлен (п.3, 2026-09-18), `frontendUrl` в
  `web.admin/src/config/sites.ts` поправлен (п.4, 2026-09-17). **Остался один
  пункт**: `NEXT_PUBLIC_SITE_URL` на Vercel Production всё ещё указывает на
  `avocado-kiss.vercel.app` (проверено живым `robots.txt` 2026-09-18, п.2 не
  сделан). Фолбэк в `lib/site.ts` (`SITE_URL`) уже переключён на
  `https://www.avocadokiss.com` — так что после того, как переменная в Vercel
  будет обновлена и пройдёт redeploy, ничего в коде менять не придётся.
  ⚠️ Отдельно от чек-листа: все code-level SEO-правки этой сессии (canonical,
  JSON-LD, OG-теги, h1/breadcrumbs, favicon, фикс слайдера — см. ниже) пока
  **только в рабочем дереве репозитория**, не закоммичены и не запушены — на
  проде их тоже нет, пока не будет коммита/пуша и деплоя.
- 🐛→✅ **`NEXT_PUBLIC_SITE_URL` на Vercel не задана** (обнаружено 2026-08-26,
  ...
  бесполезный localhost-sitemap на проде). Исправлено к 2026-09-14 (переменная
  была выставлена на `avocado-kiss.vercel.app`); 2026-09-17: вынесли дефолт в
  общий `lib/site.ts` (`SITE_URL`) и добавили `metadataBase` в `app/layout.tsx`
  для canonical/OG. Актуальный открытый пункт — обновление той же переменной
  под новый домен, см. п.2 выше и §7.1.
