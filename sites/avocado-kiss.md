...
- 🐛 **`NEXT_PUBLIC_SITE_URL` на Vercel НЕ задана** (обнаружено 2026-08-26). В
  `app/sitemap.ts` и `app/robots.ts` она читается с фолбэком
  `?? "http://localhost:3000"`, поэтому **весь прод-sitemap состоит из
  `http://localhost:3000/...`**, а `robots.txt` рекламирует
  `Sitemap: http://localhost:3000/sitemap.xml`. Для поисковиков sitemap
  бесполезен. Переменной нет и в `.env.local`. Чинится **настройкой окружения
  на Vercel** (`NEXT_PUBLIC_SITE_URL=https://avocado-kiss.vercel.app`) + редеплоем;
  правок кода не требует. К превью отношения не имеет — найдено попутно.
...
3. **Cloudflare Turnstile** — добавить новый хостнейм в настройки виджета (§11).
   ...
   рецепта, и подписка на рассылку** — форма будет отклонять отправку.
4. **`web.admin/src/config/sites.ts`** → `frontendUrl` записи `avocado-kiss`, затем
   ...
   превью-ссылки на старый домен (§2.2).
5. **Справочные значения** (не влияют на рантайм, но вводят в заблуждение):
   `avocado.kiss/.env.example`, `avocado.kiss/README.md` и упоминания прод-адреса
   в этом файле (§7).
...
> забытая переменная там не так заметна.
