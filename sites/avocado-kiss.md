...
- ⚠️ Гранты: `preview_token` анону не выдан (0020, применена 2026-08-26), поэтому
  анон-выборки `recipes`/`posts` обязаны перечислять колонки явно — `lib/columns.ts`
  (`PUBLIC_RECIPE_COLUMNS`, `PUBLIC_POST_COLUMNS`), никаких `select("*")`, включая
  embed'ы. Подробности и ловушка — [../database/schema.md](../database/schema.md) §9.
...
- `<title>`/`description` страниц `/preview/*` **не задаются** — роуты экспортируют
  статический `metadata` только с `robots`, поэтому во вкладке виден дефолтный тайтл
  layout'а, а не `seo_title`/`title` материала. Вёрстка и контент идентичны живой
  странице; отличается только тайтл вкладки. Чинится добавлением `generateMetadata`
  (тогда `robots: { index:false, follow:false }` обязан переехать внутрь неё — иначе
  потеряется noindex).
- На превью **черновика рецепта** виден блок оценки (`RatingSection`), но RPC
  `rate_recipe` гейтит `is_published = true` — клик по звезде вернёт «recipe not
  found». Для превью это косметика: страница показывается «как на сайте».
- Токен живёт в query-параметре. Кросс-доменной утечки через `Referer` нет при
  дефолтной политике браузеров (`strict-origin-when-cross-origin` шлёт только
...
- Свои картинки оптимизируются через `next/image` (хост `*.supabase.co`
  ...
  (не реализовано, зафиксировано 2026-09-17; конкретный пример — hero рецепта
  «Sourdough toast with smashed avocado and eggs», ≈2.8 MB с Unsplash).
...
- **`HeroCarousel`** (`components/HeroCarousel.tsx`, клиентский): слайды выложены
  ...
  🐛→✅ **Next с последнего слайда анимировался в обратную сторону** (найдено
  пользователем 2026-09-17, не аудитом). Причина: обработчик Next заранее
  вычислял `(active+1)%count` — клик с последнего реального слайда сразу давал
  `index=0`, минуя клон, и GSAP честно анимировал трек НАЗАД через все слайды
  (в старом коде клон/`onComplete`-снап срабатывал только у автоплея, который
  просто инкрементит `index` без модуло). Выглядело как «слайдер
  сломался»/кнопки поведения не срабатывают. **Исправлено**: Next теперь тоже
  идёт через `setIndex(i => i + 1)` (тот же путь, что автоплей) — доходит до
  клона и бесшовно снимается на слайд 0. Prev с первого слайда — ведущего
  клона нет, поэтому вместо долгой обратной прокрутки через все слайды
  сделан мгновенный снап (`instantJump` ref, без анимации). SEO Health
  Board — находка ak-22.
...
- SUPABASE-переменные нужны всегда (главная и рецепты тянут данные при сборке);
  ...
  pending).
- **Прод сейчас — `https://www.avocadokiss.com`** (кастомный домен подключён
  2026-09-17; apex `avocadokiss.com` → 308 → www, тот же паттерн, что у
  cozycorner). Домен ушёл в сторону от изначально задуманного `avocado.kiss`
  (тот так и не заработал — DNS не резолвился) — не путать упоминания
  `avocado.kiss` в старых записях этого файла с реальным доменом.
  ✅ **Чек-лист §7.1 закрыт полностью** (2026-09-18, включая
  `NEXT_PUBLIC_SITE_URL` на Vercel). Все code-level SEO-правки той сессии
  закоммичены, запушены и задеплоены — подтверждено живыми проверками
  (`canonical`, `robots.txt`, `/icon.svg`, `/contact`).
- 🐛→✅ **`NEXT_PUBLIC_SITE_URL` на Vercel не задана** (обнаружено 2026-08-26,
  фолбэк `?? "http://localhost:3000"` в `app/sitemap.ts`/`app/robots.ts` давал
  бесполезный localhost-sitemap на проде). Полностью закрыто 2026-09-18.
- ⚠️ **2026-09-18 — репозиторий перенесён на новый GitHub-аккаунт**
  (`nowmdr` → `eugeniuszlipko`, из-за лимитов Vercel Hobby-плана на аккаунте).
  Локальная настройка (SSH-алиас, remote, локальный-для-репо git identity) —
  отдельный переиспользуемый гайд:
  [methodology/github-account-per-repo.md](../methodology/github-account-per-repo.md).
  После переноса пришлось **вручную переподключать** Git-интеграцию в Vercel
  (Project → Settings → Git) — простое добавление аккаунта как коннектора
  автодеплой не включило, понадобился явный реконнект + новый пуш, чтобы
  разбудить вебхук.
- **Ветки:** `main` — прод (`www.avocadokiss.com`), `stg` — создана
  2026-09-18 как задел под стейджинг. Смэппить `stg` на отдельный домен
  (`avocado-kiss.vercel.app`, освободившийся от кастомного домена) в обход
  Production **не получилось на Hobby-плане** — Vercel привязывает домен не к
  ветке напрямую, а к Environment, а «Custom Environments» (нужны, чтобы
  завести Environment на конкретную ветку) — фича только Pro-плана. Ветка
  `stg` пока существует только в git, без отдельного домена в Vercel; вернуться
  к этому пункту, когда будет Pro.

### 7.1. ⚠️ Переезд на другой домен — чек-лист

Прод-адрес зашит в НЕСКОЛЬКИХ местах, часть из них — вне этого репозитория.
Пропуск любого пункта ломает вещи тихо, без ошибки сборки. При смене домена
(в т.ч. с временного `*.vercel.app` на кастомный) пройти всё:

1. **Vercel → Domains** — привязать новый домен к проекту.
2. **Vercel → Environment Variables** — `NEXT_PUBLIC_SITE_URL=https://<новый-домен>`.
   Со схемой, **без хвостового `/`** (склейка — `${BASE}/shop`, нормализации нет:
   слеш даст `//shop`). Затем **редеплой** — префикс `NEXT_PUBLIC_` означает
   инлайн на этапе сборки, сохранения переменной недостаточно.
   Влияет на `app/sitemap.ts` и `app/robots.ts`; фолбэк там — `http://localhost:3000`,
   поэтому забытая переменная даёт sitemap из localhost-ссылок (так и случилось,
   см. баг в §7).
3. ✅ **Cloudflare Turnstile** — добавить новый хостнейм в настройки виджета (§11).
   Иначе виджет вернёт ошибку домена (`110200`) и **перестанут работать и оценка
   рецепта, и подписка на рассылку** — форма будет отклонять отправку. Готово
   2026-09-18 (`avocadokiss.com` в allowed hostnames).
4. ✅ **`web.admin/src/config/sites.ts`** → `frontendUrl` записи `avocado-kiss`, затем
   редеплой админки. Иначе кнопка «Copy preview link» продолжит выдавать
   превью-ссылки на старый домен (§2.2). Готово 2026-09-17
   (`https://www.avocadokiss.com`) — админку саму ещё нужно передеплоить.
5. **Справочные значения** (не влияют на рантайм, но вводят в заблуждение):
   `avocado.kiss/.env.example` (✅ обновлён 2026-09-17), `avocado.kiss/README.md`
   (не проверялся) и упоминания прод-адреса в этом файле (§7, обновлено).

**Проверка после переезда:**

```
curl -s https://<новый-домен>/robots.txt                    # Sitemap: новый домен
curl -s https://<новый-домен>/sitemap.xml | grep -o '<loc>[^<]*</loc>' | head -3
```

плюс вручную: превью-ссылка из админки открывается, оценка рецепта ставится,
форма подписки принимает Turnstile.

> У cozycorner набор тот же (`NEXT_PUBLIC_SITE_URL` + `frontendUrl` в `sites.ts`),
> но фолбэк в его `sitemap.ts`/`robots.ts` — боевой URL, а не localhost, поэтому
> забытая переменная там не так заметна: до 2026-09-17 фолбэк молча указывал на
> служебный `cozycorner-omega.vercel.app` вместо канонического
> `www.cozycorner.me` (найдено тем же SEO-аудитом 2026-09-14, см. cozycorner.md
> §6) — переменной в Vercel по-прежнему нет, чинит только правильный фолбэк в
> коде.
...
## 13. Рекомендации «что дальше» — общий резолвер

Страницы **поста, рецепта и товара** собирают свои блоки рекомендаций одним
алгоритмом — `lib/relations.ts` (`resolveRelated`). Раньше это были три
несогласованные реализации: у блога были пины + авто-добор, у товара — только
пины (блок пустел без них), у рецепта не было ничего.

**Алгоритм** (одинаков для всех типов):

1. **Ручные пины** из соответствующей таблицы, по `position`. Таблицы остались
   раздельными (`post_related` / `recipe_related` / `product_pairings` /
   `product_reading`) — унифицирован слой алгоритма, а не хранилище, поэтому
   FK-каскады и RLS не переписывались. Реестр связей — константа `PIN_TABLES`.
2. **Авто-подбор по скорингу**: общая категория `+3`, каждый общий тег `+2`
   (потолок `4` — иначе теги забивают категорию), тот же тип контента `+1`;
   тай-брейк — `published_at desc`, затем `id`. Веса — именованные константы в
   начале файла. В блок идут только кандидаты со счётом > 0, то есть реально
   родственные, а не просто свежие.
3. **Случайный фолбэк** — добор опубликованными материалами, чтобы блок не был
   пустым или куцым. Рандом **детерминированный**: seed = id источника
   (FNV-1a → mulberry32 → тасование Фишера—Йетса). Иначе состав блока менялся бы
   на каждой ISR-регенерации, а тесты плавали бы. Проверено на проде: две
   независимые сборки дают побайтово одинаковые блоки на всех 30 страницах.

**Инварианты**: пины всегда первыми и в своём порядке; источник никогда не
ссылается сам на себя; дедуп по `${тип}:${id}`; выдача не длиннее `limit`.

⚠️ **`is_published` фильтруется в КОДЕ, а не через RLS** — превью (`/preview/*`)
читает service-role клиентом, который RLS обходит. Правило §2.2 действует и
здесь; покрыто тестами `lib/relations.test.ts` и `lib/preview.test.ts`.

⚠️ `targetTypes` управляет только шагами 2–3. **Ручной пин показывается всегда**,
даже если его тип не входит в `targetTypes` — закрепление это осознанный выбор
редакции. Так на странице рецепта можно закрепить товар, хотя авто-добор ведётся
рецептами и постами.

**Загрузчики-обёртки**: `fetchRelatedReading` (`lib/blog.ts`, пост),
`fetchRecipeRelated` (`lib/content.ts`, рецепт),
`fetchProductPairings` + `fetchRelatedReading` (`lib/shop.ts`, товар).
...
общего словаря нет. Поэтому в блоке «Related reading» страницы товара скоринг
почти всегда даёт 0 и блок наполняет фолбэк (случайный, но стабильный). Чтобы
подбор стал осмысленным, нужен явный маппинг «категория магазина → категория
рецептов»; сейчас его нет сознательно.

**Админка**: пины всех четырёх блоков правит один компонент
`web.admin/src/features/shared/RelationsEditor.tsx` (слой данных —
`src/lib/contentRelations.ts` с тем же реестром связей). Пустой список пинов в UI
прямо помечен как «auto-filled if empty» — это не скрытый блок.
