...
components/           # один компонент = файл + CSS Module; каталог — docs/design-system.md
  ...
  Footer.tsx          # подвал (текст из footer_settings) + NewsletterForm (реальная подписка, §7)
...
lib/
  supabase/client.ts / server.ts / admin.ts  # anon браузер/сервер + service_role (server-only, §7)
  newsletter.ts       # "use server": subscribe() — проверка Turnstile + запись подписки (§7)
...
# Форма подписки (§7). Site key вшивается в билд; secret'ы — только сервер.
NEXT_PUBLIC_TURNSTILE_SITE_KEY=…                 # публичный site key виджета Turnstile
TURNSTILE_SECRET_KEY=…                           # server-only: проверка токена Turnstile
SUPABASE_SERVICE_ROLE_KEY=sb_secret_…            # server-only: запись подписки
```

- Прод-БД — проект **`zwrkphynupdubevzwdzy`** («base-one»), не одноимённый «CozyCorner».
- SUPABASE-переменные нужны всегда (главная тянет данные при сборке); шаблон —
  `.env.example`. Все — в Vercel (Production + Preview). **Service-role вернулся**, но
  ТОЛЬКО для серверного действия подписки (§7) — для контента сайт по-прежнему на
...
## 7. Форма подписки на рассылку (Cloudflare Turnstile)

Футерная `NewsletterForm` — реальный сбор email в `cozycorner.subscribers`
(schema.md §5, миграция 0025). Поток:

1. Клиент рендерит **невидимый** виджет Turnstile (режим Invisible задан в
   дашборде Cloudflare) с отложенным запуском (`execution: "execute"`); при сабмите
   виджет тихо отдаёт токен.
2. Email + токен уходят в серверное действие `lib/newsletter.ts` (`"use server"`).
   Оно **на сервере** проверяет токен у Cloudflare (`siteverify`, `TURNSTILE_SECRET_KEY`),
   затем пишет строку через `lib/supabase/admin.ts` (**service_role**, обходит RLS).
   Дубликат email = успех (идемпотентно, `unique` по email).
3. Форма показывает статус: Subscribing… / Thanks for subscribing! / ошибка.

**Почему service_role, а не anon INSERT:** публичный anon-ключ есть в браузере;
разреши анону INSERT — бот слал бы POST в Supabase REST в обход Turnstile. Запись
только под service_role убирает публичный путь записи → Turnstile реально гейтит
каждую вставку. Таблица закрыта для анона полностью (дефолтный `select` снят
`revoke`); смотреть/удалять может только admin (`is_admin()`) — право на забвение GDPR.

**Privacy Policy:** Invisible-режим требует ссылки на Turnstile Privacy Addendum.
Добавлено в тело политики (`pages.body`, slug=`privacy`: раздел 5 «Who we share
data with» + пометка в разделе 2) и в код-фолбэк `app/privacy/page.tsx`.
⚠️ У privacy есть **DB-override** `pages.body` — правки в коде не видны, пока он не
пуст; менять текст надо в БД (через админку/SQL) ИЛИ очистить `body`.

**Ключи** (`.env.example`; в Vercel Production/Preview, затем redeploy):
`NEXT_PUBLIC_TURNSTILE_SITE_KEY` (клиент, вшивается в билд), `TURNSTILE_SECRET_KEY`
(server-only), `SUPABASE_SERVICE_ROLE_KEY` (server-only). В настройках виджета
Cloudflare добавить домен прода, иначе токен не пройдёт. Локально — тестовые
всегда-проходящие ключи Cloudflare (в `.env.example`).

## 8. Глубокие доки сайта (папка [cozycorner/](cozycorner/design-system.md) рядом с этим файлом)
